...
NetlinkRouteMessage::init(
    int type, uint32_t rtFlags, const rnl::Route& route) {
...
ResultCode
NetlinkRouteMessage::addNextHops(const rnl::Route& route) {
...
NetlinkRouteMessage::addMultiPathNexthop(
  ...
  for (const auto& path : paths) {
    ...
    if (action.hasValue()) {
      switch (action.value()) {
      case fbmeshd::thrift::MplsActionCode::PUSH:
        result = addLabelNexthop(rta, rtnh, path);
        break;

      case fbmeshd::thrift::MplsActionCode::SWAP:
      case fbmeshd::thrift::MplsActionCode::PHP:
        result = addSwapOrPHPNexthop(rta, rtnh, path);
        break;

      case fbmeshd::thrift::MplsActionCode::POP_AND_LOOKUP:
...
NetlinkRouteMessage::showMultiPathAttribues(
  ...
  VLOG(8) << "len: " << rtnh->rtnh_len << " flags: " << rtnh->rtnh_flags;
  VLOG(8) << "hop: " << rtnh->rtnh_hops << " ifindex: " << rtnh->rtnh_ifindex;
...
NetlinkRouteMessage::setMplsAction(
  ...
  if (nhBuilder.getPushLabels() != folly::none) {
    nhBuilder.setLabelAction(fbmeshd::thrift::MplsActionCode::PUSH);
  } else if (family == AF_MPLS) {
    if (nhBuilder.getGateway() != folly::none &&
        nhBuilder.getSwapLabel() != folly::none) {
      nhBuilder.setLabelAction(fbmeshd::thrift::MplsActionCode::SWAP);
    } else if (
        nhBuilder.getGateway() != folly::none &&
        nhBuilder.getSwapLabel() == folly::none) {
      nhBuilder.setLabelAction(fbmeshd::thrift::MplsActionCode::PHP);
    } else {
      nhBuilder.setLabelAction(
          fbmeshd::thrift::MplsActionCode::POP_AND_LOOKUP);
    }
  }
}
...
ResultCode
NetlinkRouteMessage::deleteRoute(const rnl::Route& route) {
  auto const& pfix = route.getDestination();
  auto addressFamily = route.getFamily();
  VLOG(8) << "Deleting route: " << route.str();
...
ResultCode
NetlinkRouteMessage::addLabelRoute(const rnl::Route& route) {
  init(RTM_NEWROUTE, 0, route);
  ...
  VLOG(8) << "Adding MPLS route " << route.str();
...
ResultCode
NetlinkRouteMessage::deleteLabelRoute(const rnl::Route& route) {
  init(RTM_DELROUTE, 0, route);
  rtmsg_->rtm_family = AF_MPLS;
  rtmsg_->rtm_dst_len = kLabelSizeBits;
  rtmsg_->rtm_flags = 0;
  ...
  VLOG(8) << "Deleting label: " << route.str();
  mlabel.entry = encodeLabel(label.value(), true);
  ResultCode status{ResultCode::SUCCESS};
  if ((status = addAttributes(
           RTA_DST,
...
NetlinkAddrMessage::addOrDeleteIfAddress(
  ...
  VLOG(8) << (type == RTM_NEWADDR ? "Adding" : "Deleting") << " IP address "
            << ifAddr.str();
  init(type);
