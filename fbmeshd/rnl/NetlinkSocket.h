...
class NetlinkSocket {
 public:
  ...
  class EventsHandler {
   public:
    ...
    linkEventFunc(
        ...
        const rnl::Link& /* linkEntry */) noexcept {
    ...
    neighborEventFunc(
        const std::string& /* ifName */,
        const rnl::Neighbor& /* neighborEntry */) noexcept {
    ...
    addrEventFunc(
        const std::string& /* ifName */,
        const rnl::IfAddress& /* addrEntry */) noexcept {
    ...
    routeEventFunc(
        const std::string& /* ifName */,
        const rnl::Route& /* routeEntry */) noexcept {
  ...
  struct EventVisitor : public boost::static_visitor<> {
    ...
    void
    operator()(rnl::Route const& route) const {
      eventHandler->routeEventFunc(linkName, route);
    }

    void
    operator()(rnl::IfAddress const& addr) const {
      eventHandler->addrEventFunc(linkName, addr);
    }

    void
    operator()(rnl::Neighbor const& neigh) const {
      eventHandler->neighborEventFunc(linkName, neigh);
    }

    void
    operator()(rnl::Link const& link) const {
  ...
  explicit NetlinkSocket(
      ...
      std::unique_ptr<rnl::NetlinkProtocolSocket> nlSock = nullptr);
 ...
 private:
  ...
  std::unique_ptr<rnl::NetlinkProtocolSocket> nlSock_{nullptr};
