...
#include <fbmeshd/if/gen-cpp2/fbmeshd_types.h>
...
using rnl::NetlinkProtocolSocket;
using rnl::NetlinkRouteMessage;
using rnl::ResultCode;
...
class NlMessageFixture : public ::testing::Test {
 ...
 protected:
  ...
  buildNextHop(
      ...
      folly::Optional<fbmeshd::thrift::MplsActionCode> action,
  ...
  buildRoute(
      ...
      const folly::Optional<std::vector<rnl::NextHop>>& nexthops) {
  ...
  buildV6RouteDb(uint32_t count) {
    std::vector<rnl::Route> routes;
    std::vector<rnl::NextHop> paths;
    paths.push_back(buildNextHop(
        outLabel4,
        folly::none,
        fbmeshd::thrift::MplsActionCode::PUSH,
        ipAddrY1V4,
        ifIndexY));
    paths.push_back(buildNextHop(
        outLabel5,
...
TEST_F(NlMessageFixture, MaxPayloadExceeded) {
  ...
  for (uint32_t i = 0; i < 200; i++) {
    ...
    paths.push_back(buildNextHop(
        outLabel5,
        folly::none,
        fbmeshd::thrift::MplsActionCode::PHP,
        ipAddress,
        ifIndexZ));
  }
...
TEST_F(NlMessageFixture, LabelRouteLabelNexthop) {
  ...
  paths.push_back(buildNextHop(
      folly::none,
      swapLabel,
      fbmeshd::thrift::MplsActionCode::SWAP,
      ipAddrY1V6,
      ifIndexZ));
  auto route = buildRoute(kRouteProtoId, folly::none, inLabel3, paths);
  ackCount = nlSock->getAckCount();
...
TEST_F(NlMessageFixture, MultipleLabelRoutes) {
  ...
  paths.push_back(buildNextHop(
      ...
      ipAddrY1V6,
      ifIndexZ));
  std::vector<rnl::Route> labelRoutes;
  for (uint32_t i = 0; i < count; i++) {
...
TEST_F(NlMessageFixture, AddrScaleTest) {
  ...
  for (int i = 0; i < addrCount; i++) {
    ...
    rnl::IfAddressBuilder ipv4builder;
